...
(function($) {
  ...
  $.fn.SPServices = function(options) {
    ...
    switch (WSops[opt.operation][0]) {
      ...
      case OFFICIALFILE:
        SOAPEnvelope.opheader +=
          "xmlns='" + SCHEMASharePoint + "/soap/recordsrepository/' >";
      ...
      case SOCIALDATASERVICE:
        ...
        SOAPAction =
          'http://microsoft.com/webservices/SharePointPortalServer/SocialDataService/';
        break;
      case SPELLCHECK:
        SOAPEnvelope.opheader +=
    ...
    if (webURL.charAt(webURL.length - 1) === SLASH) {
      ...
      ajaxURL =
        thisSite +
        (thisSite.charAt(thisSite.length - 1) === SLASH
          ? ajaxURL
          : SLASH + ajaxURL);
    }
    ...
    switch (opt.operation) {
      ...
      case 'UpdateListItems':
        ...
        if (typeof opt.updates !== 'undefined' && opt.updates.length > 0) {
          ...
          SOAPEnvelope.payload +=
            "<updates><Batch OnError='Continue'><Method ID='1' Cmd='" +
            opt.batchCmd +
            "'>";
          for (i = 0; i < opt.valuepairs.length; i++) {
            SOAPEnvelope.payload +=
              ...
              opt.valuepairs[i][0] +
              "'>" +
              escapeColumnValue(opt.valuepairs[i][1]) +
              '</Field>';
          }
          if (opt.batchCmd !== 'New') {
      ...
      case 'QueryEx':
        ...
        break;
      case 'Registration':
        SOAPEnvelope.payload += wrapNode(
          'registrationXml',
          encodeXml(opt.registrationXml)
        );
        break;
      case 'Status':
        break;
      ...
      case 'GetCommentsOnUrl':
        ...
        if (
          typeof opt.excludeItemsTime !== 'undefined' &&
          opt.excludeItemsTime.length > 0
        ) {
          SOAPEnvelope.payload += wrapNode(
            'excludeItemsTime',
            opt.excludeItemsTime
          );
        }
        break;
      ...
      case 'GetUserProfileByName':
        ...
        if (
          typeof opt.accountName !== 'undefined' &&
          opt.accountName.length > 0
        ) {
  ...
  $.fn.SPServices.SPCascadeDropdowns = function(options) {
    var opt = $.extend(
      {},
      {
        ...
        CAMLQueryOptions:
          '<QueryOptions><IncludeMandatoryColumns>FALSE</IncludeMandatoryColumns></QueryOptions>', // [Optional] For power users, ability to specify Query Options
    ...
    $().SPServices({
      ...
      completefunc: function(xData) {
        $(xData.responseXML)
          ...
          .each(function() {
            $(this)
              .find("Field[DisplayName='" + opt.childColumn + "']")
              .each(function() {
  ...
  function cascadeDropdown(parentSelect) {
    ...
    $(childColumns).each(function() {
      ...
      if (
        parentSelect.Obj.data(
          'SPCascadeDropdown_Selected_' + childColumnStatic
        ) === allParentSelections
      ) {
        return;
      }
      ...
      var sortColumn =
        opt.relationshipListSortColumn.length > 0
          ? opt.relationshipListSortColumn
          : opt.relationshipListChildColumn;
      var camlQuery =
        "<Query><OrderBy><FieldRef Name='" +
        sortColumn +
        "'/></OrderBy><Where><And>";
        ...
        completefunc: function(xData) {
            ...
            case dropdownType.simple:
              ...
              if (!childColumnRequired && opt.promptText.length > 0) {
                childSelect.Obj.append(
                  "<option value='0'>" +
                    opt.promptText.replace(/\{0\}/g, opt.childColumn) +
                    '</option>'
                );
              } else if (!childColumnRequired) {
                childSelect.Obj.append(
                  "<option value='0'>" + opt.noneText + '</option>'
                );
              }
              break;
            case dropdownType.complex:
          ...
          $(xData.responseXML)
            ...
            .each(function() {
              ...
              var thisValue = $(this).attr(
                'ows_' + opt.relationshipListChildColumn
              );

              if (
                typeof thisValue !== 'undefined' &&
                thisValue.indexOf(spDelim) > 0
              ) {
              ...
              switch (childSelect.Type) {
                case dropdownType.simple:
                  var selected =
                    $(this).attr('ows_ID') === childSelectSelected[0]
                      ? " selected='selected'"
                      : '';
                  childSelect.Obj.append(
                    '<option' +
                ...
                case dropdownType.multiSelect:
                  ...
                  newMultiLookupPickerdata +=
                    thisOption.id + '|t' + thisOption.value + '|t |t |t';
                  break;
                default:
                  break;
              }
          ...
          switch (childSelect.Type) {
            ...
            case dropdownType.multiSelect:
              ...
              $(childSelect.master.resultControl)
                .find('option')
                .each(function() {
                  var thisSelected = $(this);
                  thisSelected.prop('selected', true);
                  $(childSelect.master.candidateControl)
                    .find("option[value='" + thisSelected.val() + "']")
                    .each(function() {
              ...
              childSelect.master.data = GipGetGroupData(
                newMultiLookupPickerdata
              );
  ...
  $.fn.SPServices.SPComplexToSimpleDropdown = function(options) {
    ...
    var simpleSelectId = genContainerId(
      'SPComplexToSimpleDropdown',
      columnSelect.Obj.attr('title'),
      opt.listName
    );

    var simpleSelect =
      "<select id='" + simpleSelectId + "' title='" + opt.columnName + "'>";
    for (i = 0; i < choices.length; i = i + 2) {
      var simpleSelectSelected =
        choices[i + 1] === complexSelectSelectedId
          ? " selected='selected' "
          : ' ';
      simpleSelect +=
        '<option' +
        simpleSelectSelected +
        "value='" +
        choices[i + 1] +
        "'>" +
        choices[i] +
    ...
    simpleSelectObj.change(function() {
      ...
      $(columnSelect.Obj).val(
        $(this)
          .find("option[value='" + (thisVal !== '0' ? thisVal : '') + "']")
          .html()
      );
    });
    ...
    $().SPServices({
      ...
      completefunc: function(xData) {
        ...
        for (i = 0; i < opt.relatedColumns.length; i++) {
          relatedColumnsXML[opt.relatedColumns[i]] = $(xData.responseXML).find(
            "Fields > Field[Name='" + opt.relatedColumns[i] + "']"
          );
        }
        relatedColumnsXML[opt.relatedListColumn] = $(xData.responseXML).find(
          "Fields > Field[Name='" + opt.relatedListColumn + "']"
        );
      }
    });
  ...
  function showRelated(opt, divId, relatedListXML, relatedColumnsXML) {
    ...
    if (
      columnSelect.Type === dropdownType.complex &&
      opt.numChars > 0 &&
      columnSelectSelected[0].length < opt.numChars
    ) {
      return;
    }
    ...
    var relatedListColumnType = relatedColumnsXML[opt.relatedListColumn].attr(
      'Type'
    );
    if (relatedListColumnType === 'Lookup') {
      camlQuery +=
        "<Eq><FieldRef Name='" +
        opt.relatedListColumn +
        (opt.matchOnId
          ? "' LookupId='True'/><Value Type='Integer'>"
          : "'/><Value Type='Text'>") +
      ...
      camlQuery +=
        "<Eq><FieldRef Name='" +
        (opt.matchOnId
          ? "ID' /><Value Type='Counter'>"
          : opt.relatedListColumn + "'/><Value Type='Text'>") +
        escapeColumnValue(columnSelectSelected[0]) +
        '</Value></Eq>';
    }
      ...
      completefunc: function(xData) {
        ...
        switch (opt.displayFormat) {
          ...
          case 'table':
            ...
            for (i = 0; i < opt.relatedColumns.length; i++) {
              if (
                typeof relatedColumnsXML[opt.relatedColumns[i]] ===
                  'undefined' &&
                opt.debug
              ) {
                errBox(
              ...
              .each(function() {
                ...
                for (i = 0; i < opt.relatedColumns.length; i++) {
                  outString +=
                    ...
                    showColumn(
                      relatedListXML,
                      relatedColumnsXML[opt.relatedColumns[i]],
                      $(this).attr('ows_' + opt.relatedColumns[i]),
                      opt
                    ) +
                    '</td>';
                }
              ...
              .each(function() {
                for (i = 0; i < opt.relatedColumns.length; i++) {
                  ...
                  outString +=
                    ...
                    opt.headerCSSClass +
                    "'>" +
                    relatedColumnsXML[opt.relatedColumns[i]].attr(
                      'DisplayName'
                    ) +
                    '</th>';
                  outString +=
  ...
  $.fn.SPServices.SPFilterDropdown = function(options) {
    var opt = $.extend(
      {},
      {
        ...
        CAMLQueryOptions:
          "<QueryOptions><IncludeMandatoryColumns>FALSE</IncludeMandatoryColumns><ViewAttributes Scope='RecursiveAll'/></QueryOptions>", // Need this to mirror SharePoint's behavior, but it can be overridden
        completefunc: null, // Function to call on completion of rendering the change.
        debug: false // If true, show error messages; if false, run silent
      },
      options
    ...
    var sortColumn =
      opt.relationshipListSortColumn.length > 0
        ? opt.relationshipListSortColumn
        : opt.relationshipListColumn;
    var sortOrder =
      opt.relationshipListSortAscending === true ? '' : "Ascending='FALSE'";
    var camlQuery =
      "<Query><OrderBy><FieldRef Name='" +
      sortColumn +
      "' " +
    ...
    $().SPServices({
      ...
      completefunc: function(xData) {
        $(xData.responseXML)
          ...
          .each(function() {
            $(this)
              .find("Field[DisplayName='" + opt.columnName + "']")
              .each(function() {
    ...
    $().SPServices({
      ...
      completefunc: function(xData) {
        ...
        switch (columnSelect.Type) {
          case dropdownType.simple:
            ...
            if (!columnColumnRequired && opt.promptText.length > 0) {
              columnSelect.Obj.append(
                "<option value='0'>" +
                  opt.promptText.replace(/\{0\}/g, opt.columnName) +
                  '</option>'
              );
          ...
          case dropdownType.multiSelect:
            // Remove all of the existing options
            $(columnSelect.master.candidateControl)
              .find('option')
              .remove();
            newMultiLookupPickerdata = '';
            break;
        ...
        $(xData.responseXML)
          ...
          .each(function() {
            ...
            switch (columnSelect.Type) {
              case dropdownType.simple:
                var selected =
                  $(this).attr('ows_ID') === columnSelectSelected[0]
                    ? " selected='selected'"
                    : '';
                columnSelect.Obj.append(
                  '<option' +
        ...
        switch (columnSelect.Type) {
          ...
          case dropdownType.multiSelect:
            ...
            $(columnSelect.master.resultControl)
              .find('option')
              .each(function() {
                var thisSelected = $(this);
            ...
            // Set master.data to the newly allowable values
            columnSelect.master.data = GipGetGroupData(
              newMultiLookupPickerdata
            );
  ...
  $.fn.SPServices.SPDebugXMLHttpResult = function(options) {
    ...
    outString +=
      "<table class='ms-vb' style='margin-left:" +
      opt.indent * 3 +
      "px;' width='100%'>";
    // DisplayPatterns are a bit unique, so let's handle them differently
    if (opt.node.nodeName === 'DisplayPattern') {
      outString +=
        "<tr><td width='100px' style='font-weight:bold;'>" +
        opt.node.nodeName +
        "</td><td><textarea readonly='readonly' rows='5' cols='50'>" +
        opt.node.xml +
        '</textarea></td></tr>';
      ...
      outString +=
        ...
        opt.node.nodeName +
        '</td><td>' +
        (opt.node.nodeValue !== null
          ? checkLink(opt.node.nodeValue)
          : '&nbsp;') +
        '</td></tr>';
      if (opt.node.attributes) {
        outString +=
          "<tr><td colspan='99'>" + showAttrs(opt.node) + '</td></tr>';
      }
      // A CDATA_SECTION node
    } else if (
      opt.node.hasChildNodes() &&
      opt.node.firstChild.nodeType === NODE_CDATA_SECTION
    ) {
      outString +=
        "<tr><td width='100px' style='font-weight:bold;'>" +
        opt.node.nodeName +
        "</td><td><textarea readonly='readonly' rows='5' cols='50'>" +
        opt.node.parentNode.text +
        '</textarea></td></tr>';
      // A TEXT node
    } else if (
      opt.node.hasChildNodes() &&
      opt.node.firstChild.nodeType === NODE_TEXT
    ) {
      outString +=
        "<tr><td width='100px' style='font-weight:bold;'>" +
        opt.node.nodeName +
      ...
      outString +=
        "<tr><td width='100px' style='font-weight:bold;' colspan='99'>" +
        opt.node.nodeName +
        '</td></tr>';
      if (opt.node.attributes) {
        outString +=
  ...
  $.fn.SPServices.SPGetCurrentUser = function(options) {
    ...
    if (
      opt.fieldName === 'ID' &&
      typeof currentContext.thisUserId !== 'undefined'
    ) {
    ...
    var thisUserDisp;
    var thisWeb =
      opt.webURL.length > 0 ? opt.webURL : $().SPServices.SPGetCurrentSite();
    ...
    for (i = 0; i < fieldCount; i++) {
      ...
      if (opt.fieldNames[i] === 'ID') {
        ...
        if (fieldCount > 1) {
          thisTextValue = RegExp(
            'FieldInternalName="' + opt.fieldNames[i] + '"',
            'gi'
          );
        } else {
          thisTextValue = RegExp(
            'FieldInternalName="' + opt.fieldName + '"',
            'gi'
          );
        }
        $(thisUserDisp.responseText)
      ...
      if (opt.fieldNames[i] !== 'ID') {
        thisField =
          typeof thisField !== 'undefined'
            ? thisField.replace(/(^[\s\xA0]+|[\s\xA0]+$)/g, '')
            : null;
      }
      if (fieldCount > 1) {
  ...
  $.fn.SPServices.SPLookupAddNew = function(options) {
    ...
    $().SPServices({
      ...
      completefunc: function(xData) {
        $(xData.responseXML)
          .find("Field[DisplayName='" + opt.lookupColumn + "']")
          .each(function() {
    ...
    if (newUrl.length > 0) {
      ...
      newHref += opt.newWindow
        ? (opt.ContentTypeID.length > 0
            ? '?ContentTypeID=' + opt.ContentTypeID
            : '') + "' target='_blank'"
        : '?' +
          (opt.ContentTypeID.length > 0
            ? 'ContentTypeID=' + opt.ContentTypeID + '&'
            : '') +
          'Source=' +
          escapeUrl(location.href) +
          "'";
      var newLink =
        "<div id='SPLookupAddNew_" +
        lookupColumnStaticName +
        "'>" +
        "<a href='" +
        newHref +
        '>' +
        opt.promptText.replace(/\{0\}/g, opt.lookupColumn) +
        '</a></div>';
      // Append the link to the Lookup columns's formbody table cell
      $(lookupSelect.Obj)
        .parents('td.ms-formbody')
        .append(newLink);
    } else if (opt.debug) {
      errBox(
        thisFunction,
  ...
  $.fn.SPServices.SPGetLastItemId = function(options) {
    ...
    camlQuery +=
      "</Where><OrderBy><FieldRef Name='Created_x0020_Date' Ascending='FALSE'/></OrderBy></Query>";
  ...
  $.fn.SPServices.SPRequireUnique = function(options) {
    ...
    var msg =
      "<span id='SPRequireUnique" +
      opt.columnStaticName +
      "' class='{0}'>{1}</span><br/>";
    var firstMsg = msg
      .replace(/\{0\}/g, opt.initMsgCSSClass)
      .replace(/\{1\}/g, opt.initMsg);
    ...
    var columnObj = findFormField(columnDisplayName).find(
    ...
    columnObj.blur(function() {
      ...
      $().SPServices({
        ...
        CAMLQuery:
          "<Query><Where><IsNotNull><FieldRef Name='" +
          opt.columnStaticName +
          "'/></IsNotNull></Where></Query>",
        // Filter based on columnStaticName's value
        CAMLViewFields:
          "<ViewFields><FieldRef Name='ID' /><FieldRef Name='" +
          opt.columnStaticName +
          "' /></ViewFields>",
        // Override the default view rowlimit and get all appropriate rows
        CAMLRowLimit: 0,
        completefunc: function(xData) {
          var testValue = opt.ignoreCase
            ? columnValue.toUpperCase()
            : columnValue;
          $(xData.responseXML)
            .SPFilterNode('z:row')
            .each(function() {
              var thisValue = opt.ignoreCase
                ? $(this)
                    .attr('ows_' + opt.columnStaticName)
                    .toUpperCase()
      ...
      $("input[value='OK']:disabled, input[value='Save']:disabled").removeAttr(
        'disabled'
      );
      if (columnValueIDs.length > 0) {
        ...
        if (opt.duplicateAction === 1) {
          columnObj.focus();
          $("input[value='OK'], input[value='Save']").attr(
            'disabled',
            'disabled'
          );
        }
        if (opt.showDupes) {
          var out =
            ' ' +
            columnValueIDs.length +
            ' duplicate item' +
            (columnValueIDs.length > 1 ? 's' : '') +
            ': ';
          for (i = 0; i < columnValueIDs.length; i++) {
            out +=
              "<a href='DispForm.aspx?ID=" +
              columnValueIDs[i][0] +
              '&Source=' +
              location.href +
              "'>" +
  ...
  $.fn.SPServices.SPGetDisplayFromStatic = function(options) {
    ...
    var displayNames = {};
    var nameCount =
      opt.columnStaticNames.length > 0 ? opt.columnStaticNames.length : 1;
      ...
      completefunc: function(xData) {
        if (nameCount > 1) {
          for (i = 0; i < nameCount; i++) {
            displayNames[opt.columnStaticNames[i]] = $(xData.responseXML)
              .find("Field[StaticName='" + opt.columnStaticNames[i] + "']")
              .attr('DisplayName');
          }
        } else {
          displayName = $(xData.responseXML)
            .find("Field[StaticName='" + opt.columnStaticName + "']")
            .attr('DisplayName');
        }
      }
    });
  ...
  $.fn.SPServices.SPGetStaticFromDisplay = function(options) {
    ...
    var staticNames = {};
    var nameCount =
      opt.columnDisplayNames.length > 0 ? opt.columnDisplayNames.length : 1;
    ...
    $().SPServices({
      ...
      completefunc: function(xData) {
        if (nameCount > 1) {
          for (i = 0; i < nameCount; i++) {
            staticNames[opt.columnDisplayNames[i]] = $(xData.responseXML)
              .find("Field[DisplayName='" + opt.columnDisplayNames[i] + "']")
              .attr('StaticName');
          }
        } else {
          staticName = $(xData.responseXML)
            .find("Field[DisplayName='" + opt.columnDisplayName + "']")
            .attr('StaticName');
        }
      }
    });
  ...
  $.fn.SPServices.SPRedirectWithID = function(options) {
    ...
    if (typeof queryStringVals.ID === 'undefined') {
      ...
      $("form[id='aspnetForm']").each(function() {
        ...
        var thisSource =
          typeof queryStringVals.Source === 'string'
            ? 'Source=' +
              queryStringVals.Source.replace(/\//g, '%2f').replace(/:/g, '%3a')
            : '';
        ...
        var newAction =
          ...
          (thisSource.length > 0
            ? '%26RealSource=' + queryStringVals.Source
            : '') +
          // Pass the override RedirectURL, if present
          (typeof queryStringVals.RedirectURL === 'string'
            ? '%26RedirectURL=' + queryStringVals.RedirectURL
            : '');
      ...
      location.href =
        ...
        '=' +
        lastID +
        (typeof queryStringVals.RealSource === 'string'
          ? '&Source=' + queryStringVals.RealSource
          : '');
    }
  ...
  $.fn.SPServices.SPSetMultiSelectSizes = function(options) {
    ...
    if (thisMultiSelect.Obj.html() === null && opt.debug) {
      errBox(
        thisFunction,
        'multiSelectColumn: ' + opt.multiSelectColumn,
        TXTColumnNotFound
      );
      return;
    }
    ...
    var cloneId = genContainerId(
      'SPSetMultiSelectSizes',
      opt.multiSelectColumn,
      opt.listName
    );
    var cloneObj = $("<select id='" + cloneId + "' ></select>").appendTo(
      thisMultiSelect.container
    );
    cloneObj.css({
    ...
    $(thisMultiSelect.master.candidateControl)
      .find('option')
      .each(function() {
        cloneObj.append(
          "<option value='" +
    ...
    $(thisMultiSelect.master.resultControl)
      .find('option')
      .each(function() {
        cloneObj.append(
          "<option value='" +
  ...
  $.fn.SPServices.SPScriptAudit = function(options) {
    ...
    $('#' + opt.outputId).append(
      "<table id='SPScriptAudit' width='100%' style='border-collapse: collapse;' border=0 cellSpacing=0 cellPadding=1>" +
        '<tr>' +
        '<th></th>' +
        '<th>List</th>' +
        '<th>Page Class</th>' +
        ...
        completefunc: function(xData) {
          $(xData.responseXML)
            ...
            .each(function() {
              ...
              if (
                opt.listName.length === 0 ||
                listXml.attr('Title') === opt.listName
              ) {
                // Don't work with hidden lists unless we're asked to
                if (
                  (opt.showHiddenLists && listXml.attr('Hidden') === 'False') ||
                  !opt.showHiddenLists
                ) {
                          ...
                          .each(function() {
                            ...
                            if (
                              ...
                              for (i = 0; i < formTypes.length; i++) {
                                ...
                                if (!formTypes[i][2]) {
                                  var defaultViewUrl = listXml.attr(
                                    'DefaultViewUrl'
                                  );
                                  SPScriptAuditPage(
                                    opt,
                                    ...
                                    'Form',
                                    formTypes[i][0],
                                    defaultViewUrl.substring(
                                      0,
                                      defaultViewUrl.lastIndexOf(SLASH) + 1
                                    ) + formTypes[i][1]
                                  );
                                }
                              }
    ...
    if (opt.auditPages) {
      for (i = 0; i < numLists; i++) {
        ...
        $().SPServices({
          ...
          listName: listsArray[i],
          CAMLQuery:
            "<Query><Where><Neq><FieldRef Name='ContentType'/><Value Type='Text'>Folder</Value></Neq></Where></Query>",
          CAMLViewFields:
            "<ViewFields><FieldRef Name='Title'/><FieldRef Name='FileRef'/></ViewFields>",
          CAMLRowLimit: 0,
          completefunc: function(xData) {
            $(xData.responseXML)
              .SPFilterNode('z:row')
              .each(function() {
                var thisPageUrl = $(this)
                  .attr('ows_FileRef')
                  .split(spDelim)[1];
                var thisTitle = $(this).attr('ows_Title');
                var thisPageType =
                  typeof thisTitle !== 'undefined' ? thisTitle : '';
    ...
    scriptAuditContainer
      .find("tr[class='ms-alternating']:even")
      .removeAttr('class');
      ...
      success: function(xData) {
        ...
        while ((scriptMatch = scriptRegex.exec(xData))) {
          ...
          if (
            scriptSrc !== null &&
            scriptSrc.length > 0 &&
            !coreScript(scriptSrc)
          ) {
            pageScriptSrc.type.push(
              scriptLanguage !== null && scriptLanguage.length > 0
                ? scriptLanguage
                : scriptType
            );
        ...
        if (
          (!opt.showNoScript && pageScriptSrc.type.length > 0) ||
          opt.showNoScript
        ) {
          var pagePath = pageUrl.substring(0, pageUrl.lastIndexOf(SLASH) + 1);
          var out =
            '<tr class=ms-alternating>' +
            "<td class=ms-vb-icon><a href='" +
            listXml.attr('DefaultViewUrl') +
            "'><IMG border=0 src='" +
            listXml.attr('ImageUrl') +
            "'width=16 height=16></A></TD>" +
            "<td class=ms-vb2><a href='" +
            listXml.attr('DefaultViewUrl') +
            "'>" +
            listXml.attr('Title') +
            (listXml.attr('Hidden') === 'True' ? '(Hidden)' : '') +
            '</td>' +
            '<td class=ms-vb2>' +
            pageClass +
            '</td>' +
          ...
          if (opt.showSrc) {
            var thisSrcPath;
            out +=
              "<td valign='top'><table width='100%' style='border-collapse: collapse;' border=0 cellSpacing=0 cellPadding=1>";
            for (i = 0; i < pageScriptSrc.type.length; i++) {
              thisSrcPath =
                pageScriptSrc.src[i].substr(0, 1) !== SLASH
                  ? pagePath + pageScriptSrc.src[i]
                  : pageScriptSrc.src[i];
              out +=
                "<tr><td class=ms-vb2 width='30%'>" +
                pageScriptSrc.type[i] +
                '</td>';
              out +=
                "<td class=ms-vb2 width='70%'><a href='" +
                thisSrcPath +
                "'>" +
                fileName(pageScriptSrc.src[i]) +
                '</td></tr>';
            }
            out += '</table></td>';
          }
  ...
  $.fn.SPServices.SPArrangeChoices = function(options) {
    ...
    thisGetList.done(function() {
      ...
      if (columnFillInChoice) {
        var fillInRow = $(
          "<tr><td colspan='99'><table cellpadding='0' cellspacing='1'><tr></tr></table></td></tr>"
        );
  ...
  $.fn.SPServices.SPAutocomplete = function(options) {
    ...
    var columnObj = findFormField(opt.columnName).find(
      "input[Title^='" + opt.columnName + "']"
    );
    ...
    var containerId = genContainerId(
      'SPAutocomplete',
      opt.columnName,
      opt.listName
    );
    columnObj.after(
      "<div><ul id='" +
        ...
        "' style='width:" +
        columnObjWidth +
        ";display:none;padding:2px;border:1px solid #2A1FAA;background-color:#FFF;position:absolute;z-index:40;margin:0'></div>"
    );
        ...
        completefunc: function(xData) {
          ...
          $(xData.responseXML)
            ...
            .each(function() {
              ...
              var thisValueTest = opt.ignoreCase
                ? $(this)
                    .attr('ows_' + opt.sourceColumn)
                    .toUpperCase()
                : $(this).attr('ows_' + opt.sourceColumn);
              ...
              if (opt.filterType === 'Contains') {
                ...
                if (
                  testValue === thisValueTest.substr(0, testValue.length) &&
                  // ...and that the match is not already in the array if we want uniqueness
                  (!opt.uniqueVals || $.inArray(thisValue, matchArray) === -1)
                ) {
                  matchArray.push($(this).attr('ows_' + opt.sourceColumn));
        ...
        if (opt.highlightClass.length > 0) {
          ...
          for (matchNum = 0; matchNum < matches.length; matchNum++) {
            ...
            var thisSpan =
              ...
              '</span>';
            matchArray[i] =
              matchArray[i].substr(0, thisPos) +
              thisSpan +
              matchArray[i].substr(endPos);
            startLoc = thisPos + thisSpan.length;
          }
        }
        // Add the value to the markup for the container
        out +=
          "<li style='display: block;position: relative;cursor: pointer;'>" +
          matchArray[i] +
          '</li>';
      }
  ...
  $.fn.SPServices.SPListNameFromUrl = function(options) {
    ...
    if (opt.listName.length > 0) {
      ...
      currentContext.thisList !== undefined &&
      currentContext.thisList.length > 0
    ) {
      return currentContext.thisList;
    }
    ...
    var listPath = decodeURIComponent(
      thisPageBaseName.substring(0, thisPageBaseName.lastIndexOf(SLASH) + 1)
    ).toUpperCase();
      ...
      completefunc: function(xData) {
        $(xData.responseXML)
          ...
          .each(function() {
            ...
            var listCollList = defaultViewUrl
              .substring(0, defaultViewUrl.lastIndexOf(SLASH) + 1)
              .toUpperCase();
  ...
  $.fn.SPServices.SPUpdateMultipleListItems = function(options) {
    ...
    for (i = 0; i < itemsToUpdate.length; i++) {
      ...
      for (fieldNum = 0; fieldNum < opt.valuepairs.length; fieldNum++) {
        batch +=
          "<Field Name='" +
          opt.valuepairs[fieldNum][0] +
          "'>" +
          escapeColumnValue(opt.valuepairs[fieldNum][1]) +
          '</Field>';
      }
  ...
  $.fn.SPServices.SPGetListItemsJson = function(options) {
    ...
    thisData.done(function() {
      ...
      responseXml
        .find("listitems Changes Id[ChangeType='Delete']")
        .each(function() {
      ...
      for (attrNum = 0; attrNum < rowAttrs.length; attrNum++) {
        ...
        var thisObjectName =
          typeof thisMapping !== 'undefined'
            ...
            : opt.removeOws
            ? thisAttrName.split('ows_')[1]
            : thisAttrName;
        var thisObjectType =
          typeof thisMapping !== 'undefined'
            ? thisMapping.objectType
            : undefined;
        if (opt.includeAllAttrs || thisMapping !== undefined) {
          row[thisObjectName] = attrToJson(
  ...
  function userMultiToJsonObject(s) {
    if (s.length === 0) {
      ...
      for (i = 0; i < thisUserMulti.length; i = i + 2) {
        var thisUser = userToJsonObject(
          thisUserMulti[i] + spDelim + thisUserMulti[i + 1]
        );
  ...
  function lookupMultiToJsonObject(s) {
    if (s.length === 0) {
      ...
      for (i = 0; i < thisLookupMulti.length; i = i + 2) {
        var thisLookup = lookupToJsonObject(
          thisLookupMulti[i] + spDelim + thisLookupMulti[i + 1]
        );
      ...
      .filter(function() {
        ...
        return (
          $(this)
            .contents()
            .eq(0)
            .text() === opt.peoplePickerDisplayName
        );
      })
      .closest('tr');
  ...
  $.fn.SPServices.SPDropdownCtl = function(options) {
    ...
    if (
      (columnObj.Obj = $("select[Title='" + opt.displayName + "']")).length ===
      1
    ) {
      columnObj.Type = dropdownType.simple;
      // Compound
    } else if (
      (columnObj.Obj = $("input[Title='" + opt.displayName + "']")).length === 1
    ) {
      ...
      (columnObj.Obj = $(
        "select[ID$='SelectCandidate'][Title^='" + opt.displayName + " ']"
      )).length === 1
    ) {
    ...
    if (columnObj.Type === null) {
      ...
      if (fieldContainer !== undefined) {
        var fieldSelect = fieldContainer.find(
          "select[title^='" + opt.displayName + " '][id$='_Lookup']"
        );
    ...
    if (columnObj.Type === dropdownType.complex) {
      ...
      columnObj.MultiLookupPickerdata = columnObj.container.find(
        "input[id$='" +
          multiLookupPrefix +
          "_data'], input[id$='" +
          multiLookupPrefix2013 +
          "_data']"
      );
      var addButtonId = columnObj.container
        .find("[id$='AddButton']")
        .attr('id');
      columnObj.master =
  ...
  function SPServicesContext() {
    ...
    if (typeof _spPageContextInfo !== 'undefined') {
      ...
      this.thisSite =
        typeof L_Menu_BaseUrl !== 'undefined' ? siteRoot + L_Menu_BaseUrl : '';
      this.thisList = '';
      this.thisUserId =
        typeof _spUserId !== 'undefined' ? _spUserId : undefined;
    }
    ...
    switch (columnXML.attr('Type')) {
      ...
      case 'URL':
        switch (columnXML.attr('Format')) {
          ...
          case 'Hyperlink':
            outString =
              "<a href='" +
              columnValue.substring(0, columnValue.search(',')) +
              "'>" +
              columnValue.substring(columnValue.search(',') + 1) +
              '</a>';
            break;
          // URL as image
          case 'Image':
      ...
      case 'UserMulti':
        ...
        for (i = 0; i < userMultiValues.length; i = i + 2) {
          outArray.push(
            "<a href='/_layouts/userdisp.aspx?ID=" +
              userMultiValues[i] +
              '&Source=' +
              escapeUrl(location.href) +
              "'>" +
      ...
      case 'Lookup':
        switch (columnXML.attr('Name')) {
          case 'FileRef':
            ...
            dispUrl =
              listXML.attr('BaseType') === '1'
                ? listXML.attr('RootFolder') + SLASH + 'Forms/DispForm.aspx'
                : listXML.attr('RootFolder') + SLASH + 'DispForm.aspx';
            outString =
              "<a href='" +
              dispUrl +
              '?ID=' +
              columnValue.substring(0, columnValue.search(spDelim)) +
              '&RootFolder=*&Source=' +
              escapeUrl(location.href) +
              "'>" +
              columnValue.substring(columnValue.search(spDelim) + 2) +
              '</a>';
            break;
          case 'FileDirRef':
            // Get the display form URL for the lookup source list
            dispUrl =
              SLASH + columnValue.substring(columnValue.search(spDelim) + 2);
            outString =
              "<a href='" +
              dispUrl +
              "'>" +
          ...
          for (i = 0; i < lookupMultiValues.length / 2; i++) {
            outArray.push(
              "<a href='" +
                ...
                dispUrl +
                '?ID=' +
                lookupMultiValues[i * 2] +
                '&RootFolder=*&Source=' +
                escapeUrl(location.href) +
                "'>" +
                lookupMultiValues[i * 2 + 1] +
                '</a>'
            );
          }
        }
  ...
  function showAttrs(node) {
    ...
    for (i = 0; i < node.attributes.length; i++) {
      out +=
        "<tr><td width='10px' style='font-weight:bold;'>" +
        i +
        "</td><td width='100px'>" +
        node.attributes.item(i).nodeName +
        '</td><td>' +
        checkLink(node.attributes.item(i).nodeValue) +
        '</td></tr>';
    }
    out += '</table>';
    return out;
  ...
  function getDropdownSelected(columnSelect, matchOnId) {
    ...
    switch (columnSelect.Type) {
      case dropdownType.simple:
        if (matchOnId) {
          columnSelectSelected.push(
            columnSelect.Obj.find('option:selected').val() || []
          );
        } else {
          columnSelectSelected.push(
            columnSelect.Obj.find('option:selected').text() || []
          );
        }
        break;
      case dropdownType.complex:
  ...
  function modalBox(msg) {
    var boxCSS =
      "position:absolute;width:300px;height:150px;padding:10px;background-color:#000000;color:#ffffff;z-index:30;font-family:'Arial';font-size:12px;display:none;";
    $('#aspnetForm')
      .parent()
      .append("<div id='SPServices_msgBox' style=" + boxCSS + '>' + msg);
  ...
  function genContainerId(funcname, columnName, listName) {
    var l =
      listName !== undefined ? listName : $().SPServices.SPListNameFromUrl();
    return (
      funcname +
      '_' +
  ...
  function addToPayload(opt, paramArray) {
    ...
    for (i = 0; i < paramArray.length; i++) {
      ...
      if (typeof paramArray[i] === 'string') {
        ...
        typeof paramArray[i] === 'object' &&
        paramArray[i].sendNull !== undefined
      ) {
        SOAPEnvelope.payload +=
          opt[paramArray[i].name] === undefined ||
          opt[paramArray[i].name].length === 0
            ? ''
  ...
  function siteDataFixSOAPEnvelope(SOAPEnvelope, siteDataOperation) {
    ...
    SOAPEnvelope.opheader = SOAPEnvelope.opheader.replace(
      siteDataOperation,
      siteDataOp
    );
    SOAPEnvelope.opfooter = SOAPEnvelope.opfooter.replace(
      siteDataOperation,
      siteDataOp
    );
    return SOAPEnvelope;
  ...
  function checkLink(s) {
    return s.indexOf('http') === 0 || s.indexOf(SLASH) === 0
      ? "<a href='" + s + "'>" + s + '</a>'
      : s;
  }
